...
"""Provides a class, defaults, and utils for Melody SNN model configuration."""

from magenta.contrib import training as contrib_training
from magenta.models.melody_snn import melody_snn_model
import note_seq
import tensorflow.compat.v1 as tf

FLAGS = tf.app.flags.FLAGS
# TODO: add poisson encoding, sample & average decoders
...
class MelodySnnConfigError(Exception):
  pass
...
def config_from_flags():
  """Parses flags and returns the appropriate MelodySnnConfig.

  If `--config` is supplied, returns the matching default MelodySnnConfig after
  updating the hyperparameters based on `--hparams`.

  If `--melody_encoder_decoder` is supplied, returns a new MelodySnnConfig using
  ...
  Returns:
    The appropriate MelodySnnConfig based on the supplied flags.

  Raises:
     MelodySnnConfigError: When not exactly one of `--config` or
         `melody_encoder_decoder` is supplied.
  """
  if (FLAGS.melody_encoder_decoder, FLAGS.config).count(None) != 1:
    raise MelodySnnConfigError(
  ...
  if FLAGS.melody_encoder_decoder is not None:
    ...
    encoder_decoder = melody_encoder_decoders[FLAGS.melody_encoder_decoder](
        melody_snn_model.DEFAULT_MIN_NOTE, melody_snn_model.DEFAULT_MAX_NOTE)
    hparams = contrib_training.HParams()
    hparams.parse(FLAGS.hparams)
    return melody_snn_model.MelodySnnConfig(
        generator_details, encoder_decoder, hparams)
  else:
    if FLAGS.config not in melody_snn_model.default_configs:
      raise MelodySnnConfigError(
          '`--config` must be one of %s. Got %s.' % (
              melody_snn_model.default_configs.keys(), FLAGS.config))
    config = melody_snn_model.default_configs[FLAGS.config]
