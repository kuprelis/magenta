...
from magenta.models.melody_snn import events_snn_graph
...
class EventSequenceSnnModelError(Exception):
  pass
...
class EventSequenceSnnModel(model.BaseModel):
  """Class for SNN event sequence generation models.
  ...
  def __init__(self, config):
    """Initialize the EventSequenceSnnModel.

    Args:
      config: An EventSequenceSnnConfig containing the encoder/decoder and
        HParams to use.
    """
    super(EventSequenceSnnModel, self).__init__()
    self._config = config
  ...
  def _generate_events(self, num_steps, primer_events, temperature=1.0,
    ...
    Raises:
      EventSequenceSnnModelError: If the primer sequence has zero length or
    ...
    if (control_events is not None and
      ...
      raise EventSequenceSnnModelError(
  ...
  def _evaluate_log_likelihood(self, event_sequences, control_events=None):
    ...
    Raises:
      EventSequenceSnnModelError: If the event sequences are not all the
...
class EventSequenceSnnConfig(object):
  """Stores a configuration for an event sequence SNN.
